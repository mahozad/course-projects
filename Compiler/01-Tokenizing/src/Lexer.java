...
class Lexer {
    ...
    private int addNextToken(String phrase) {
        ...
        switch (phrase.charAt(0)) {
            ...
            case '}':
            case ';':
                return addUnaryToken(tokenList, caret, "SEPARATOR", phrase
                        .substring(0, 1));
            case '+':
            case '-':
            case '/':
            case '*':
                return addUnaryToken(tokenList, caret, "OPERATOR", phrase
                        .substring(0, 1));
            case '=':
            case '!':
            case '<':
            ...
            case '&':
            case '|':
                return addBinaryToken(phrase, caret);
            default:
        ...
        switch (nextToken) {
            ...
            default:
                if (validateAsIdentifier(nextToken)) {
                    ...
                    ERRORS.add("Line " + currentLine + ": illegal character");
                }
                break;
        }
        return caret;
    ...
    private int addUnaryToken(List<Token> tokenList, int caret, String
            lexemeClass, String lexemeName) {
        tokenList.add(new Token(lexemeName, lexemeClass, currentLine));
        return ++caret;
    }

    private int addBinaryToken(String phrase, int caret) {
        if (phrase.charAt(1) == '=') {
            caret += 2;
